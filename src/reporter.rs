...
fn write_requirements(results: &[PackageResult], path: &Path) -> anyhow::Result<()> {
    ...
    writeln!(file, "# Packages with NO FIX are commented out — review manually\n")?;
...
fn write_markdown_report(results: &[PackageResult], path: &Path) -> anyhow::Result<()> {
    ...
    writeln!(f, "> Generated by [cpv](https://github.com/michaelcolletti/cpv)\n")?;
    ...
    writeln!(f, "| Clean packages | {} |\n", results.len() - vulnerable.len())?;

    // Remediation summary table
    writeln!(f, "## Remediation Plan\n")?;
    writeln!(f, "| Package | Current Version | Safe Version | Action | Downstream Dependents |")?;
    writeln!(f, "|---------|----------------|-------------|--------|----------------------|")?;
    ...
    if transitive.is_empty() {
        ...
        for (r, deps) in &transitive {
            ...
            writeln!(f, "- **Used by:** {}", deps.iter().map(|d| format!("`{}`", d)).collect::<Vec<_>>().join(", "))?;
            writeln!(f, "- **Action:** Upgrade `{}`, then verify dependents still resolve correctly.", r.package.name)?;
            writeln!(f)?;
        }
    }
    ...
    for r in &vulnerable {
        ...
        for v in &r.vulns {
            ...
            if let Some(sev) = v.severity.first() {
                writeln!(f, "- **Severity:** `{}` score `{}`", sev.severity_type, sev.score)?;
            }
